...
if __name__ == "__main__":
    ...
    # 定义3个变量
    sql = """
        select * from douban_movie_send as A LEFT JOIN douban_movie ON A.`data-subject`=douban_movie.`data-subject` ORDER BY id DESC LIMIT 1;
    """
    cursor.execute(sql)
    result = cursor.fetchall()

    title = str(result[0][2])
    score = str(result[0][3])
    actors = str(result[0][8])
    link = "http://movie.douban.com/subject/" + str(result[0][14])
    ...
    for phone in phones:
        sender.sendMsg("86", phone, "电影名称《" + title + "》,评分" + score + "分，主演：" + actors +" " + link)
